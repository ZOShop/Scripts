...
PerformHttpRequest('https://api.ipify.org/?format=json', function(errorCode2, resultData2, resultHeaders2)
	...
	while not resultData2 do
		...
		PerformHttpRequest('https://api.ipify.org/?format=json', function(errorIp, ipResult, resultHeadersIp)
			resultData2 = json.decode(ipResult)	
		end)
	end
    
    PerformHttpRequest('http://54.39.11.213:3000/ip/buscarips/' .. idUser, function(errorCode, resultData, resultHeaders)
        if resultData == nil then
            PerformHttpRequest('http://54.39.11.213:3000/ip/buscarips/', function(errorCode3, resultData3, resultHeaders3)
